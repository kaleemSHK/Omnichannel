...
import { useUpdateEscalationRule } from '@/lib/hooks/useEscalation';
import {
  ESCALATION_TRIGGERS,
  apiActionsToUi,
...
const ACTION_PLACEHOLDER: Record<UiActionType, string> = {
  ...
  notify: '#sla-alerts',
  label: 'urgent or priority:high',
  message: 'Internal note text',
  webhook: 'https://hooks.example.com/escalation',
};

export function EditRuleModal({ rule, open, onClose }: Props) {
  ...
  const [actions, setActions] = useState<UiActionRow[]>([{ type: 'label', target: '' }]);
  ...
  useEffect(() => {
    ...
    setConditions({ conditions: rule.conditions, logic: rule.conditionLogic });
    setActions(apiActionsToUi(rule.actions));
  }, [rule, open]);
  ...
  return (
    <Dialog open={open} onClose={onClose} title="Edit escalation rule" className="max-w-xl">
