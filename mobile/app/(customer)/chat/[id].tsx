import { useCallback, useEffect, useState } from 'react';
import { View, FlatList, Text, ActivityIndicator, Alert } from 'react-native';
import { useLocalSearchParams } from 'expo-router';
import { SafeAreaView } from 'react-native-safe-area-context';
import { useQueryClient } from '@tanstack/react-query';
import { AppHeader } from '@/components/layout/AppHeader';
import { MessageBubble } from '@/components/chat/MessageBubble';
import { MessageInput } from '@/components/chat/MessageInput';
import { useMessages } from '@/hooks/useMessages';
import { useActionCable } from '@/hooks/useActionCable';
import { createContact } from '@/api/contacts';
import { createConversation, listInboxes } from '@/api/conversations';
import { loadCustomerSession, saveCustomerSession } from '@/lib/storage';

export default function CustomerChatScreen() {
  const { id } = useLocalSearchParams<{ id: string }>();
  const queryClient = useQueryClient();
  const [conversationId, setConversationId] = useState<number>(() =>
    id && id !== 'new' ? Number(id) : 0,
  );
  const [bootstrapping, setBootstrapping] = useState(id === 'new');

  const { messages, isLoading, send, refetch } = useMessages(conversationId);

  const onRealtime = useCallback(() => {
    queryClient.invalidateQueries({ queryKey: ['messages', conversationId] });
  }, [queryClient, conversationId]);

  useActionCable(conversationId, { onMessage: onRealtime });

  useEffect(() => {
    if (id !== 'new' || conversationId > 0) return;
    let cancelled = false;
    (async () => {
      try {
        const session = await loadCustomerSession();
        if (session.conversationId) {
          setConversationId(session.conversationId);
          setBootstrapping(false);
          return;
        }
        const inboxes = await listInboxes();
        const inbox = inboxes[0];
        if (!inbox) throw new Error('No inbox available');
        let contactId = session.contactId;
        if (!contactId) {
          const contact = await createContact({ name: 'Mobile Customer' });
          contactId = contact.id;
          await saveCustomerSession({ contactId });
        }
        const conv = await createConversation(inbox.id, contactId);
        if (cancelled) return;
        await saveCustomerSession({ conversationId: conv.id, contactId });
        setConversationId(conv.id);
      } catch (e) {
        if (!cancelled) {
          Alert.alert('Chat unavailable', e instanceof Error ? e.message : 'Could not start chat');
        }
      } finally {
        if (!cancelled) setBootstrapping(false);
      }
    })();
    return () => {
      cancelled = true;
    };
  }, [id, conversationId]);

  return (
    <SafeAreaView className="flex-1 bg-bg" edges={['top']}>
      <AppHeader title="Support Chat" />
      {bootstrapping || (isLoading && conversationId === 0) ? (
        <View className="flex-1 items-center justify-center">
          <ActivityIndicator color="#63b3ed" />
        </View>
      ) : (
        <>
          <FlatList
            data={[...messages].reverse()}
            inverted
            keyExtractor={(item) => String(item.id)}
            renderItem={({ item }) => <MessageBubble message={item} />}
            contentContainerStyle={{ padding: 16 }}
            onRefresh={refetch}
            refreshing={false}
          />
          <MessageInput onSend={send} disabled={conversationId <= 0} />
        </>
      )}
    </SafeAreaView>
  );
}
