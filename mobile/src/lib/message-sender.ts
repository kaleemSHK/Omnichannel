...
export function parseMessageType(
  ...
  return resolveMessageSender({ message_type: raw as string | number | undefined, sender }) === 'customer' ? 0 : 1;
}

export function normalizeMessage<T extends Record<string, unknown>>(raw: T): T {
...
export function normalizeMessages<T extends Record<string, unknown>>(items: T[]): T[] {
  return items.map(m => normalizeMessage(m));
}
