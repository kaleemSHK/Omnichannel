...
export async function runMigrations(log = console) {
  ...
  for (const file of readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith('.sql')).sort()) {
    const { rows } = await p.query('SELECT 1 FROM billing_schema_migrations WHERE name = $1', [file]);
    ...
    try {
      ...
      await client.query('INSERT INTO billing_schema_migrations (name) VALUES ($1)', [file]);
      await client.query('COMMIT');
      log.info?.({ file }, 'billing migration applied');
    } catch (e) {
