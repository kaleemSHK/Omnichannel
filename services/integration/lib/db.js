...
export async function runMigrations(log = console) {
  ...
  for (const file of readdirSync(MIGRATIONS_DIR).filter((f) => f.endsWith('.sql')).sort()) {
    const { rows } = await p.query('SELECT 1 FROM integration_schema_migrations WHERE name = $1', [file]);
    ...
    try {
      ...
      await client.query('INSERT INTO integration_schema_migrations (name) VALUES ($1)', [file]);
      await client.query('COMMIT');
      log.info?.({ file }, 'integration migration applied');
    } catch (e) {
...
export { tenantQuery, withTenantClient } from './pg-tenant.js';
